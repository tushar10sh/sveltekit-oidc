<script context="module" lang="ts">
	...
	export async function login(oidcPromise) {
		try {
			const { session, issuer, redirect, page } = await oidcPromise;
	...
	export async function logout(oidcPromise, post_logout_redirect_uri) {
		const { issuer } = await oidcPromise;
...
<script lang="ts">
	...
	const oidc_auth_promise = Promise.resolve({
		redirect: initiateFrontChannelOIDCAuth(browser, oidcBaseUrl, client_id, scope, redirect_uri).redirect,
		session: $session,
		issuer,
		page: $page
	});
	...
	setContext(OIDC_CONTEXT_POST_LOGOUT_REDIRECT_URI, import.meta.env.VITE_OIDC_POST_LOGOUT_REDIRECT_URI);
